...
Este script realiza un Análisis de Sentimientos sobre opiniones escritas en español, utilizando Deep Learning, PyTorch y la librería Hugging Face Transformers. El archivo CSV debe contener opiniones en español, cada una en una fila. El flujo incluye: carga y preprocesamiento de datos, tokenización,  construcción de dataset y dataloader, carga de un modelo preentrenado (BERT), entrenamiento (opcional), evaluación y predicción de sentimientos. Los comentarios de cada sección explican detalladamente los pasos realizados.
...
model_name = "dccuchile/bert-base-spanish-wwm-cased"                             # Modelo BERT preentrenado para español
```
...
Se utiliza un tokenizador compatible con el modelo BERT en español para convertir textos en tensores.
```
...
class OpinionesDataset(Dataset):                                                 # Dataset personalizado
    def __init__(self, texts, labels):                                           # Inicialización con textos y etiquetas
        encodings = tokenize_texts(texts)                                        # Tokeniza los textos
        self.encodings = encodings                                               # Almacena los tensores de entrada
        self.labels = torch.tensor(labels)                                       # Convierte las etiquetas a tensores

    def __getitem__(self, idx):                                                  # Método para obtener un elemento
        item = {key: val[idx] for key, val in self.encodings.items()}            # Obtiene los tensores de un índice
        item["labels"] = self.labels[idx]                                        # Añade la etiqueta correspondiente
        return item                                                              # Retorna el diccionario

    def __len__(self):                                                           # Retorna la cantidad de ejemplos
...
## Carga del modelo preentrenado para clasificación de secuencias

Se utiliza un modelo BERT en español con una capa de clasificación para sentimientos.
```
model = AutoModelForSequenceClassification.from_pretrained(
    model_name,                                                                  # Nombre del modelo preentrenado
    num_labels=2                                                                 # Número de clases de salida (negativo, positivo)
)
```
...
def compute_metrics(eval_pred):                                                  # Función para métricas
    ...
    return {"accuracy": accuracy}                                                # Retorna un diccionario con la métrica
```

## Configuración del entrenamiento

Se definen los argumentos de entrenamiento, como número de épocas, batch size, y directorio de salida.
```
...
```
trainer = Trainer(
    model=model,                                                                 # El modelo BERT preentrenado
...
```
## Entrena el modelo (descomente la línea siguiente para entrenar)
```
...
def predecir_sentimiento(texto):                                                 # Función de predicción
    inputs = tokenize_texts([texto])                                             # Tokeniza el texto de entrada
    outputs = model(**inputs)                                                    # Pasa el texto por el modelo
    pred = torch.argmax(outputs.logits, dim=1).item()                            # Obtiene la clase predicha
    return "Positivo" if pred == 1 else "Negativo"                               # Devuelve la etiqueta
```
## Ejemplo de uso
```
